 ...
 * All code © 2014 Essemble Ltd. all rights reserved
...
DragDrop.prototype = {
	...
	initDrags:function(){
		...
		for(var i=0; i<this._arDrags.length; i++){
			...
			$(drag.container).css("color", "black");
	...
	submit:function(vars){
		...
		for (var i=0;i<this._arDrags.length;i++) {
			...
			if(drag.match != -1){
				...
				if(bCorrect){
					$(drag.container).css("color","green");
					$(drag.container).addClass( "true" );
					userCorrectCount++;
				}
				else{
					$(drag.container).css("color","red");
					$(drag.container).addClass( "false" );
					}
			}
		}
	...
	sca:function(vars){
		...
		for(var i=0; i<this._arDrags.length; i++){
			drag = this._arDrags[i];
			$(drag.container).css("color", "black");
			$(drag.container).removeClass( "true" );$(drag.container).removeClass( "false" );
	...
	reset:function(vars){
		...
		//reset the drags
		for (var i=0;i<this._arDrags.length;i++) {
			var drag = this._arDrags[i];$(drag.container).removeClass( "true" );$(drag.container).removeClass( "false" );
			this.snapBack(drag);
